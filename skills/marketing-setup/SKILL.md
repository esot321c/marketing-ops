...
- `data/` — the live workspace the app reads and writes. Seed it by copying the shape of `data.example/` (tenants, setup, accounts, guides, research, calendar, content). Per-tenant init state lives at `data/setup/<tenant>/init.json`.
...
**Agent writes:** `data/setup/<tenant>/intake.md` (structured intake notes) and uploads to `data/setup/<tenant>/assets/`.
